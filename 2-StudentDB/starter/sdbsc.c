...
{
    off_t write_pos = id * STUDENT_RECORD_SIZE;
    student_t student = {0};
    ...
    // last check if the student is all zeros, it was deleted previously
...
{
    student_t student = {0}; // new student
    off_t write_pos;
    ...
    // grading note:  its OK if they just use strcpy for now, but call out
    // if they actually check the length
    student.id = id;
    student.gpa = gpa;
    strncpy(student.fname, fname, sizeof(student.fname));
    strncpy(student.lname, lname, sizeof(student.lname));

    write_pos = student.id * STUDENT_RECORD_SIZE;
...
{
    student_t student = {0}; // empty student
    off_t write_pos;
...
int count_db_records(int fd)
{
    ...
    // staring at beginning of file, keep reading till end or error
    while (1)
    {
        ...
        // we have a student at this point, but remember if a student was deleted
        // their id would be 0
        if (student.id == DELETED_STUDENT_ID)
            continue;

        // we have a record we want to count
        record_count++;
    }

    // if we hit here and first_row still true then the DB is empty
    if (record_count == 0)
    {
        printf(M_DB_EMPTY);
    }
...
int print_db(int fd)
{
    ...
    while (1)
    {
        ...
        // remember, gpa is an init between 0..500 to convert to a real
        //  x.xx gpa divide by 100.0
        gpa = student.gpa / 100.0;
...
{
    // handle if student id is zero by just ignoring
    if ((s == NULL) || (s->id == 0))
    {
 ...
 *
 */
#define __EXTRA_CREDIT__ // use this to test both extra credit and
                         // not extra credit options
#ifndef __EXTRA_CREDIT__
...
#endif
#ifdef __EXTRA_CREDIT__
int compress_db(int fd)
{

    int tmp_fd;
    int bytes_read;
    student_t tmp_student = {0};
    int copied_records = 0;
    ...
    while (1)
    {
        bytes_read = read(fd, &tmp_student, STUDENT_RECORD_SIZE);
        if (bytes_read == 0)
        { // eof
            break;
        }
        ...
        // skip a student that has been deleted, or if this is just a logical
        // hole in the file
        if (tmp_student.id == 0)
            continue;

        // now we can add the student, using add_student() will produce I/O
        // so we will recreate it here
        int write_pos = tmp_student.id * STUDENT_RECORD_SIZE;
        if (lseek(tmp_fd, write_pos, SEEK_SET) < 0)
        {
            printf(M_ERR_DB_READ);
            close(tmp_fd);
            unlink(TMP_DB_FILE);
        ...
        ssize_t bytes_written = write(tmp_fd, &tmp_student, STUDENT_RECORD_SIZE);
    ...
    // now that the files is closed, use the rename system call to rename
    // the temporary file to the real database file.
    ...
    // last requirement is to open the newly compressed database file so
    // that we can return its file descriptor
    fd = open_db(DB_FILE, false);
    if (fd < 0)
    {
        return fd; // opendb prints the required output
    }
